...
function BookCar() {
  // Initialize provider/signer/contract only when window.ethereum is available
  const getContract = () => {
  ...
  const confirmBooking = async (e) => {
    ...
    if (
      ...
      try {
        const carRentalContract = getContract();
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        ...
        if (!selectedCar) {
          ...
          const paymentAmount = ethers.utils.parseUnits(totalCost.toFixed(2), 'ether');
          const gasLimit = 300000;
  
          const transaction = await signer.sendTransaction({
            to: carRentalContract.address,
            value: paymentAmount,
            gasLimit: gasLimit,
          });
